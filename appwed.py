...
COLORES = {
    "Pizzas": {"bg": "#2b5c8f", "text": "#ffffff"},  # Azul
    "Alitas": {"bg": "#c69214", "text": "#ffffff"},  # Amarillo
    "Hamburguesas": {"bg": "#d97724", "text": "#ffffff"},  # Naranja
    "Entradas": {"bg": "#2e7d32", "text": "#ffffff"},  # Verde
    "Otros": {"bg": "#0284c7", "text": "#ffffff"},  # Celeste
}
...
st.markdown(
    ...
    /* 3. BOTONES DE PRODUCTOS (Garantiza mínimo 80px de alto y color fijo activo) */
    ...
    /* 4. BOTONES PRINCIPALES DEL CARRITO (Arriba) */
    ...
    /* 5. DISPLAY DE TOTAL EN VERDE GIGANTE (28-36px) */
    .total-box {{
...
# --- BASE DE DATOS Y LÓGICA EXISTENTE ---
def inicializar_bd():
...
with tab1:
  ...
  if not caja_activa:
    st.warning(
        "⚠️ La caja está CERRADA. Abre caja en la pestaña 'Control de Caja' para"
        " iniciar ventas."
    )
    col_open1, col_open2 = st.columns(2)
    with col_open1:
      clave_apertura = st.text_input(
          "Contraseña de Administrador", type="password", key="pass_open"
      )
    with col_open2:
    ...
    # DISTRIBUCIÓN PANTALLA (Grilla Productos vs Carrito)
    ...
    with col_menu:
      ...
      with st.container(key="prod_zone"):
        ...
        for i, (p_id, p_nom, p_precio) in enumerate(prods):
          ...
          with col:
            # Botón grande (Mínimo 80px alto por CSS)
            if st.button(
                f"{p_nom}\nS/ {p_precio:.2f}",
                key=f"p_{p_id}",
    ...
    with col_carrito:
      ...
      # Cálculo previo del total
      total_calculado = sum(
          item["precio"] * item["cant"]
          for item in st.session_state.carrito.values()
      )

      # Mover Acciones Principales (COBRAR y VACIAR) a la Parte Superior
      col_actions_1, col_actions_2 = st.columns([2, 1])

      with col_actions_1:
        btn_cobrar_click = st.button(
            "💰 COBRAR", key="btn_cobrar", use_container_width=True
        )
      with col_actions_2:
        if st.button(
            "🗑 VACIAR", key="btn_vaciar", use_container_width=True
        ):
          st.session_state.carrito = {}
          st.rerun()

      # DISPLAY DE TOTAL GIGANTE VERDE
      st.markdown(
          f'<div class="total-box">TOTAL: S/ {total_calculado:.2f}</div>',
          unsafe_allow_html=True,
      )
      ...
      if btn_cobrar_click:
        if not st.session_state.carrito:
          ...
          c = conn.cursor()
          c.execute(
              "INSERT INTO ventas (correlativo, total, metodo, fecha, detalle)"
              " VALUES (?, ?, ?, ?, ?)",
              (
                  correlativo,
                  total_calculado,
      ...
      # LISTA DETALLADA DEL CARRITO CON AGRUPACIÓN
      if st.session_state.carrito:
        ...
        for p_id, item in list(st.session_state.carrito.items()):
          ...
          # Fila agrupada con controles (+, -, 🗑)
          c_info, c_m, c_cant, c_p, c_del = st.columns([3, 0.8, 0.8, 0.8, 0.8])

          with c_info:
            st.markdown(
                f"**{item['nombre']}**\n"
                f"<small>Unit: S/ {item['precio']:.2f} | Sub: S/"
                f" {subtotal:.2f}</small>",
                unsafe_allow_html=True,
            )
      ...
      # EMISIÓN E IMPRESIÓN DE TICKET DE LA ÚLTIMA VENTA
...
with tab3:
  ...
  if clave == PIN_ADMIN:
    ...
    if caja_activa:
      ...
      tot_yape = (
          df_ventas[df_ventas["metodo"] == "Yape"]["total"].sum()
          if not df_ventas.empty
          else 0
      )
      tot_plin = (
          df_ventas[df_ventas["metodo"] == "Plin"]["total"].sum()
          if not df_ventas.empty
          else 0
      )
      tot_mixto = (
          df_ventas[df_ventas["metodo"] == "Mixto"]["total"].sum()
          if not df_ventas.empty
          else 0
      )
      ...
      st.metric(
          ...
          f"S/ {(caja_activa[1] + tot_efectivo):.2f}",
      )
